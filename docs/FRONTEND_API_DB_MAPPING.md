...
| Portfolio/Dashboard | 계좌 금액·손익·보유 metadata·당일 기여·자산 이력은 실제 API, 산식 미구현 AI 평가는 unavailable | `GET /api/v1/accounts/me`, `GET /portfolio?account_id=`, `GET /portfolio/history`, `GET /orders`, `GET /executions` | `virtual_accounts`, `positions`, `portfolio_snapshots`, `strategy_target_weights`, `market_stocks`, `market_indices` + Redis/KIS 현재가 |
| StockDetail | 실제 현재가·차트·종목/재무 요약, AI 평가는 unavailable | `GET /api/v1/market/stocks/{code}/price`, `GET /market/stocks/{code}/summary`, `GET /market/stocks/{code}/chart` | `/price`만 Redis/KIS 현재가를 조회하고 `/summary`는 KRX `market_*`·OpenDART `company_*`만 사용; 1D 분봉은 KIS, 현재가 실패 상태는 DB 기반 요약과 분리 |
