...
| Portfolio/Dashboard | 홈 통합 조회, 최근 체결·상세 거래내역, 실제 계좌 평가·당일 기여·자산 이력·5축 feature·리밸런싱 판단 이력 | `GET /api/v1/accounts/me`, `GET /portfolio/home`, `GET /portfolio/transactions`, `GET /portfolio`, `GET /portfolio/history`, `GET/POST /portfolio/decisions`, `GET /portfolio/stock-evaluation` | `virtual_accounts`, `positions`, `executions`, `market_stocks`, `portfolio_snapshots`, `strategy_target_weights`, `rebalancing_decisions`, KRX/OpenDART + Redis/KIS 현재가 |
...
React → FastAPI /portfolio/home → account + portfolio evaluation + snapshot history
