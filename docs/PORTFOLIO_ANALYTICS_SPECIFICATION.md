...
KRX 종가는 수정주가가 아니므로 상장주식수가 직전 유효 관측과 달라진 날은 액면분할·병합 등
corporate action 경계로 보고 수익률을 계산하지 않는다. 해당 종가는 다음 수익률의 새 기준가로
연결한다. 이 정책은 기존 Strategy Backtest와 동일하다.
...
- 판단 요청 시점에 서버가 다시 평가한 포트폴리오 총자산과 그 가격 기준일
...
없으므로 만들지 않는다. 판단 기준일보다 이후 스냅샷이 없으면 결과는 null이다. 판단마다
경과 기간이 다르므로 최근 6개월 요약은 수락·보류 건수만 제공하고 성과 평균은 제공하지 않는다.
