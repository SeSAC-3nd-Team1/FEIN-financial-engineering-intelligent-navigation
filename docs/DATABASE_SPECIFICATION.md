...
`20260816_0010`은 과거 금융/API PostgreSQL `raw`와 `processed` schema retirement를 migration history에 공식 기록한다. `20260816_0011`은 회원가입 구조를 3NF 기준으로 확장/정리하고, `20260823_0012`는 가상거래를, `20260824_0013`은 OpenDART serving table을, `20260824_0014`는 투자성향·전략 추천 이력을, `20260824_0015`는 가상투자 시작 상태를, `20260824_0016`은 KRX 화면 조회용 serving table을 추가한다. `20260825_0019`는 주문·체결·보유수량을 `numeric(20,8)`로 확장해 가상 소수점 매매를 허용하고, `20260825_0020`은 운용방식별 가상계좌와 부족분 1회 입금 이력을 추가한다. `20260825_0021`은 계좌 데이터를 변경하지 않는 사용자별 활성 운용방식 선택을 추가하며, `20260826_0022`는 이메일 단독 인증 가입을 위해 `users.phone_verified_at`의 필수 제약을 해제한다. `20260828_0025`는 전략 카탈로그 분류와 실행 엔진 메타데이터를 추가하고, `20260828_0026`은 전략과 독립적인 계좌 현금 입금 기록을 추가한다. `20260828_0027`은 revision만 기록되고 `0025` DDL이 누락된 DB를 forward-only 방식으로 현재 전략 카탈로그 계약에 맞게 복구한다. `20260828_0028`은 `investor_profile_assessments.risk_score`를 추가하며, 원본 답변이 없어 역산할 수 없는 기존 v1 기록은 `NULL`로 보존한다. `20260828_0029`는 홈 화면 "목표 차량" 위젯의 등급·목표/현재 금액을 계정 단위로 저장하는 `user_car_goals`를 추가한다(원래 `0025`였으나 같은 날짜에 독립적으로 만들어진 `20260828_0025`(전략 카탈로그)와 revision id가 충돌해 `0029`로 재배치했고, 이미 옛 `0025`로 테이블이 만들어진 공용 DB에서는 존재 확인 후 건너뛰는 멱등 DDL을 쓴다).
