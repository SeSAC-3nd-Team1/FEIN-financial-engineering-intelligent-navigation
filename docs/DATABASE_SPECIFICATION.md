...
Strategy Backtest는 별도 결과 테이블이나 합성 시세를 만들지 않고 이 serving table을 읽기 전용으로 사용한다. 시작일 직전 시총으로 universe를 고정하고 factor 계산에는 해당 리밸런싱일 이하의 가격만 사용한다. 가치 factor에 필요한 재무 공시 가능일이 현재 `company_financials`에 없으므로 최신 재무를 과거에 소급 적용하지 않는다.
