...
3. 각 종목의 OHLCV를 팀 원본 `output/Algorithm(ver.2.3).py`에 그대로 전달한다.
...
유동성 정렬은 여러 종목 중 실행 대상을 정하기 위한 운영 규칙이며 투자 신호에는 사용하지 않는다. 매수 노출, 레짐 판단, 위험 축소는 모두 `Algorithm(ver.2.3)`의 출력에서 가져온다.
