...
저장된 최근 253개 공통 거래일의 종목 종가와 KOSPI 데이터를 사용해 최초 매수와 월간
리밸런싱을 날짜순으로 재생합니다. 최근 분기에는 모멘텀 신호에 따라 삼성전자와
SK하이닉스를 각각 20% 비중으로 편입합니다. 주문·체결·현금 원장·최종 포지션·일별
스냅샷은 하나의 transaction으로 저장합니다. 같은 명령을 다시 실행하면 기존 데모 계정을
반환하며 중복 거래를 생성하지 않습니다.
