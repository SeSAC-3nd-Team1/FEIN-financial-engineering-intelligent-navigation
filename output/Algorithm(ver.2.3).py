"""
Algorithm(ver.2.3).py
===================
BOCPD + HMM (Markov Switching) + LightGBM -> regime experts -> BMA -> fractional Kelly
-> core/satellite -> unified risk state -> durable checkpoint/reconciliation -> execution.
...
실행 예시
---------
    python "Algorithm(ver.2.3).py"
    python "Algorithm(ver.2.3).py" --csv prices.csv --state-file state.json
...
def restore_runtime_checkpoint(
    ...
    if payload.get("strategy_version") != "2.3":
    ...
    strategy.bma.log_reliability = {
        str(k): float(v) for k, v in dict(strategy_state["bma_log_reliability"]).items()
    }
...
def load_loss_cut_monitor(
    ...
    """같은 폴더의 ``Algorithm(ver.2.3)_loss_cut.py``를 동적으로 로드한다."""
    ...
    module_name = "algorithm_ver_2_3_loss_cut"
