...
class BayesianModelAverager:
    ...
    def weights(
        ...
        # 마지막 normalize로 cap이 다시 깨지는 문제가 없다.
        bounded = {name: self.floor for name in weights}
        free = set(weights)
        remaining = 1.0 - count * self.floor
        while free and remaining > EPS:
            denominator = sum(weights[name] for name in free)
            if denominator <= EPS:
                shares = {name: remaining / len(free) for name in free}
            else:
                shares = {
                    name: remaining * weights[name] / denominator
                    for name in free
                }
            capped = {
                name for name in free
                if bounded[name] + shares[name] > self.cap + EPS
            }
            if not capped:
                for name in free:
                    bounded[name] += shares[name]
                remaining = 0.0
                break
            for name in capped:
                allocation = self.cap - bounded[name]
                bounded[name] = self.cap
...
class PaperBroker:
    ...
    def rebalance_at_open(
        ...
        current_notional = self.state.units * open_price
        rate = self.cost_model.one_way_rate
        naive_notional = target_weight * equity_before
        if naive_notional >= current_notional:
            # N = w * (E - r * (N - N0))
            desired_notional = target_weight * (equity_before + rate * current_notional) / (
                1.0 + target_weight * rate
            )
        else:
            # N = w * (E - r * (N0 - N))
            desired_notional = target_weight * (equity_before - rate * current_notional) / max(
                1.0 - target_weight * rate, EPS
            )
        desired_units = desired_notional / open_price
