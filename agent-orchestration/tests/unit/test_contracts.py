...
from agent_orchestration.contracts import AgentReport, CoordinatorPlan, extract_json_object
...
def test_coordinator_plan_rejects_unconfigured_agent_role():
    with pytest.raises(ValidationError):
        CoordinatorPlan(
            request_id="request-1",
            selected_roles=["FundamentalAnalyst"],
            tasks={"FundamentalAnalyst": "재무 분석"},
        )
