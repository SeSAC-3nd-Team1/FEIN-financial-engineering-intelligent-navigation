...
class MarketService:
    ...
    def get_quote(self, stock_code: str) -> CurrentQuote:
            ...
            if realtime_cached:
                ...
                if age <= settings.realtime_price_stale_seconds:
                    return CurrentQuote(
                        ...
                        price=quote.price,
                        previous_close=quote.price - quote.change if quote.price - quote.change > 0 else None,
        ...
        try:
            ...
            if cached:
                ...
                return CurrentQuote(
                    ...
                    previous_close=Decimal(payload["previous_close"]) if payload.get("previous_close") is not None else None,
                    change_amount=Decimal(payload["change_amount"]) if payload.get("change_amount") is not None else None,
                    change_rate=Decimal(payload["change_rate"]) if payload.get("change_rate") is not None else None,
        ...
        try:
            self.cache.setex(key, settings.price_cache_ttl_seconds, json.dumps({
                "price": str(quote.price),
                "previous_close": str(quote.previous_close) if quote.previous_close is not None else None,
                "change_amount": str(quote.change_amount) if quote.change_amount is not None else None,
                "change_rate": str(quote.change_rate) if quote.change_rate is not None else None,
                "volume": quote.volume,
...
def _positive_ratio(numerator: Decimal | None, denominator: Decimal | None, multiplier: Decimal = Decimal("1")) -> Decimal | None:
...
class StockMarketService:
    ...
    def __init__(self, repository: MarketDataRepository, live_market: MarketService | None = None) -> None:
    ...
    def summary(self, stock_code: str) -> StockSummaryResponse:
        ...
        if company:
            description_parts.append(f"{company.corp_name}은(는) {stock.market} 상장 기업입니다.")
            if company.established_date:
                description_parts.append(f"설립일은 {company.established_date.isoformat()}입니다.")
            if company.industry_code:
                description_parts.append(f"OpenDART 업종 코드는 {company.industry_code}입니다.")
    ...
    def chart(self, stock_code: str, period: str) -> StockChartResponse:
        if period == "1D":
            ...
            return StockChartResponse(
                ...
                items=[StockChartItemResponse(
                    date=item.started_at.isoformat(), open=item.open, high=item.high,
        ...
        return StockChartResponse(
            ...
            items=[StockChartItemResponse(
                date=item.trade_date.isoformat(), open=item.open_price, high=item.high_price,
                low=item.low_price, close=item.close_price, volume=item.volume,
            ) for item in prices],
        )
