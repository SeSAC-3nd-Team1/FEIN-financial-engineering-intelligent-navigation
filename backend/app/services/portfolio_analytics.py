...
def _linear_score(value: float, low: float, high: float, *, inverse: bool = False) -> int:
...
def _axis(key: str, label: str, score: int | None, basis: str) -> StockEvaluationAxisResponse:
...
def calculate_stability(stock_returns: dict[date, float]) -> tuple[int | None, str]:
    ...
    return score, f"최근 {len(stock_returns)}거래일 연환산 변동성 {annualized_volatility * 100:.1f}%를 사용했습니다."


def calculate_financial_health(financial) -> tuple[int | None, str]:
    required = (financial.total_assets, financial.total_equity, financial.operating_cash_flow)
    ...
    score = round(
        (_linear_score(equity_ratio, 0.0, 0.70) + _linear_score(cash_flow_ratio, -0.05, 0.10)) / 2
    )
    return score, f"{financial.business_year}년 자본비율 {equity_ratio * 100:.1f}%와 영업현금흐름/자산 {cash_flow_ratio * 100:.1f}%를 사용했습니다."


def calculate_growth(latest, previous) -> tuple[int | None, str]:
    values = (latest.revenue, latest.operating_income, previous.revenue, previous.operating_income)
    if any(value is None for value in values) or previous.revenue <= 0 or previous.operating_income <= 0:
    ...
    score = round(
        (_linear_score(revenue_growth, -0.20, 0.20) + _linear_score(operating_growth, -0.20, 0.20)) / 2
    )
    return score, f"{previous.business_year}→{latest.business_year} 매출 {revenue_growth * 100:+.1f}%, 영업이익 {operating_growth * 100:+.1f}%를 사용했습니다."


def calculate_defense(
    stock_returns: dict[date, float], benchmark_returns: dict[date, float]
) -> tuple[int | None, str]:
    dates = [day for day in stock_returns.keys() & benchmark_returns.keys() if benchmark_returns[day] < 0]
    ...
    return score, f"최근 공통 KOSPI 하락일 {len(dates)}개의 하락 포착률 {downside_capture * 100:.1f}%를 사용했습니다."
...
def calculate_diversification(
    ...
    if len(dates) < MIN_PRICE_RETURNS:
        return None, f"다른 보유종목과 겹치는 수익률 표본이 {MIN_PRICE_RETURNS}개 미만입니다."
    ...
    return score, f"다른 보유종목의 시가가중 수익률과 상관계수 {coefficient:.2f}를 사용했습니다."
...
class PortfolioAnalyticsService:
    ...
    def stock_evaluation(self, user_id: int, account_id: UUID, stock_code: str) -> StockEvaluationResponse:
        ...
        financial_health = (
            ...
            if annuals else (None, "최신 연간 OpenDART 재무제표가 없습니다.")
        )
        growth = (
            calculate_growth(annuals[0], annuals[1])
            if len(annuals) >= 2 else (None, "비교 가능한 2개 연도의 OpenDART 재무제표가 없습니다.")
        )
        ...
        other_returns = self._other_portfolio_returns(account_id, stock_code, start_date)
        diversification = calculate_diversification(stock_returns, other_returns)

        targets = self.trading.target_weights(account.selected_strategy_id, latest_price.trade_date) if account.selected_strategy_id and latest_price else {}
        ...
        role_summary = None
        if strongest:
            target_text = f" 전략 목표 비중은 {(target_weight * 100):.1f}%입니다." if target_weight is not None else ""
        ...
        return StockEvaluationResponse(
            ...
            target_weight=(target_weight * 100).quantize(Decimal("0.01")) if target_weight is not None else None,
            role_summary=role_summary,
            axes=axes,
            sources=sources,
        )

    def _other_portfolio_returns(self, account_id: UUID, excluded_code: str, start_date: date) -> dict[date, float]:
        ...
        result = {}
        for day in dates:
            available = [(value, returns[day]) for value, returns in series if day in returns]
            covered = sum(value for value, _ in available)
            if covered >= total_weight * 0.7:
                result[day] = sum(value * daily_return for value, daily_return in available) / covered
        return result
    ...
    def record_decision(
        ...
        if not account:
            raise NotFoundError("ACCOUNT_NOT_FOUND", "계좌를 찾을 수 없습니다.")
        existing = self.trading.decision_by_idempotency(request.account_id, request.idempotency_key)
        if existing:
            return self._decision_response(existing)
        portfolio = PortfolioService(self.session).evaluate(user_id, request.account_id)
        proposal = next(
            (item for item in portfolio.rebalancing_proposals if item.stock_code == request.stock_code),
            None,
        )
        if proposal is None:
    ...
    def decision_history(self, user_id: int, account_id: UUID) -> RebalancingDecisionHistoryResponse:
    ...
    def _decision_response(self, decision: RebalancingDecision) -> RebalancingDecisionResponse:
        ...
        if (
            ...
            actual_return = ((latest.total_assets / decision.baseline_total_assets - 1) * 100).quantize(Decimal("0.01"))
