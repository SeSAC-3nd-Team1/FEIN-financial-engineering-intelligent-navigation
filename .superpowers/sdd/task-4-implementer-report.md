...
Implemented the universe provider and fail-closed financial guardrails from the approved Task 4 brief, including the required review remediations and re-review hardening.
...
2. Confirmed RED with the supplied Python interpreter: copy-update and mutability tests failed against the prior implementation, including the reproduced `KeyError` path.
3. Added the minimal boundary hardening and confirmed GREEN: `25 passed` for `tests/unit/test_guardrails.py`.

## Verification

- Focused tests: `25 passed`.
- Full test suite: `45 passed` within the 60-second verification bound.
