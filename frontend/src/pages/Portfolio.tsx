...
export default function Portfolio({ userName, onNavigate, onOpenDetail }: Props) {
  ...
  return (
    ...
    <>
    {/* 뷰포트를 넘어가는 순간 스크롤이 생기므로, 페이지 전체를 h-screen 으로 고정하고
        넘치는 콘텐츠는 자르는 대신(overflow-hidden) 아래 flex 트리를 전부 가용 높이에 맞춰 나눈다. */}
    <div className="flex h-screen flex-col overflow-hidden bg-canvas">
      ...
      <main className="flex min-h-0 flex-1 flex-col items-center overflow-hidden px-16 pb-6 pt-4">
        <div className="flex min-h-0 w-[1040px] flex-1 flex-col">
          {/* "나의 포트폴리오" 카드 — 제목/탭은 카드 상단에 걸치고, 그 아래는 항상
              좌(차트) : 우(투자 정보) = 1:1 그리드로 고정한다(grid-cols-2). */}
          <section className="flex min-h-0 flex-1 flex-col gap-3 rounded-card bg-surface p-8">
            ...
            {/* 좌(차트) : 우(투자 정보) 1:1 그리드 — grid-cols-2 는 두 컬럼을 항상 정확히 절반씩 나눈다 */}
            <div className="grid min-h-0 flex-1 grid-cols-2 gap-6">
              {/* 왼쪽 컬럼(50%) — 현재 탭의 차트. "자세히" 버튼은 이 컬럼의 우하단 모서리에 고정한다
                  (relative + absolute, 카드 전체가 아니라 차트 컬럼 기준 좌표). */}
              <div className="relative flex min-h-0 flex-col gap-3 pb-14">
                {tab === 'trend' && (
                ...
                {/* "자세히" — 클릭 시 보유종목/AI제안/거래내역 등 전체 관리 화면(portfolio-detail)으로 라우팅한다.
                    왼쪽(차트) 컬럼의 우하단 모서리에 고정 배치한다(요구사항: bottom-right of the donut/chart section). */}
                <button
                  onClick={onOpenDetail}
                  className="absolute bottom-0 right-0 rounded-field bg-lime px-6 py-3 text-sm font-bold text-navy shadow-[0_4px_16px_rgba(24,36,58,0.18)]"
                >
                  자세히 →
                </button>
              </div>

              {/* 오른쪽 컬럼(50%) — 투자 정보 + 향후 위젯을 위한 예약 공간. 탭과 무관하게 항상 동일하게 보여준다. */}
              {/* self-center — grid 행의 기본 stretch 를 끄고 콘텐츠 높이만큼만 차지하게 하면서(빈 여백 없음),
                  왼쪽 도넛(justify-center 로 컬럼 안에서 수직 중앙 정렬됨)과 같은 수직 중심선에 맞춘다. */}
              <div className="flex min-h-0 flex-col gap-4 self-center rounded-[20px] bg-canvas px-8 py-6">
