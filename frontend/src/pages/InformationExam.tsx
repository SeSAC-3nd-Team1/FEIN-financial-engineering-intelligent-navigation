...
export default function InformationExam({ userName, onNavigate }: Props) {
  ...
  const load = useCallback(
    async (target: InfoTab, force = false) => {
      ...
      try {
        ...
        setError(e instanceof Error ? e.message : '네트워크 연결을 확인한 뒤 다시 시도해주세요.');
      } finally {
        setLoading(false);
      }
    },
  ...
  return (
    <div className="min-h-screen bg-canvas">
      <Header active="information" userName={userName} onNavigate={onNavigate} />
        ...
        <div className="flex w-[1040px] flex-col gap-10">
          <section className="flex flex-col gap-4">
            ...
            <h1 className="text-[44px] font-bold leading-[62px] tracking-[-0.035em]">
              오늘 시장에서<br />알아두면 좋은 것들
            </h1>
            <p className="max-w-[820px] text-[19px] leading-8 text-muted">
              투자 판단에 도움이 되는 뉴스와, 어려운 용어를 쉬운 말로 풀어둔 글을 모았어요.
            </p>
          </section>

          <div className="flex items-center justify-between gap-6">
            <div className="flex items-center gap-2 rounded-full bg-[#F4F6F1] p-1.5">
              <TabBtn active={tab === 'news'} onClick={() => setTab('news')}>한국 뉴스</TabBtn>
              <TabBtn active={tab === 'knowledge'} onClick={() => setTab('knowledge')}>금융 상식</TabBtn>
            </div>
            <div className="flex items-center gap-4">
              {updatedAt && <span className="text-[15px] text-subtle">{relTime(updatedAt)} 업데이트</span>}
              <button
          ...
          {error && (
            <div className="flex flex-col items-center gap-5 rounded-card bg-surface px-10 py-20">
              ...
              <h2 className="text-2xl font-bold tracking-[-0.025em]">지금은 정보를 불러올 수 없어요</h2>
              <p className="max-w-[520px] text-center text-[17px] leading-7 text-muted">{error}</p>
              <button onClick={() => void load(tab, true)} className="rounded-field bg-lime px-8 py-4 text-[17px] font-bold text-navy">
                다시 시도하기
              </button>
            </div>
          )}
          ...
          {empty && (
            <div className="flex flex-col items-center gap-4 rounded-card bg-surface px-10 py-20">
              <h2 className="text-2xl font-bold tracking-[-0.025em]">아직 보여드릴 글이 없어요</h2>
              <p className="text-[17px] leading-7 text-muted">잠시 뒤에 새로운 소식이 채워질 거예요.</p>
            </div>
          )}
                ...
                >
                  ...
                  <div className="flex min-w-0 flex-1 flex-col gap-2.5">
                    ...
                    <div className="flex items-center gap-2.5 pt-0.5 text-[15px] text-subtle">
                      <span className="font-semibold text-[#3F4A43]">{a.publisher}</span>
                      <span>·</span>
              ...
              {knowledge!.map((k) => (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <span className="rounded-full bg-[#F1FBD4] px-3 py-1.5 text-sm font-bold text-[#3F5222]">{k.category}</span>
                    <span className="text-sm text-subtle">{k.readingMinutes}분 읽기</span>
                  </div>
                  <span className="text-[22px] font-bold leading-[34px] tracking-[-0.025em] [text-wrap:pretty]">{k.title}</span>
                  <span className="flex-1 text-base leading-[27px] text-muted">{k.excerpt}</span>
                  <span className="text-base font-semibold text-navy">읽어보기 →</span>
                </a>
              ))}
            </div>
          )}
          ...
          <p className="text-sm leading-[22px] text-subtle">
            ※ 외부 매체가 제공하는 정보이며, 특정 종목의 매매를 권유하지 않습니다.
          </p>
        </div>
      </main>
    </div>
