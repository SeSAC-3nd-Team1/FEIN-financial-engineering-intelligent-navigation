...
const TIMEFRAMES: { label: string; period: StockChartPeriod }[] = [
  { label: '1일', period: '1D' }, { label: '1주', period: '1W' },
  { label: '3개월', period: '3M' }, { label: '6개월', period: '6M' },
  { label: '1년', period: '1Y' }, { label: '5년', period: '5Y' },
];

export default function StockDetail({ stockCode, userName, onNavigate, onBack }: Props) {
        ...
        <div className="flex w-[1040px] flex-col gap-8">
          ...
          <section className="flex items-end justify-between gap-8">
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-baseline gap-4">
                ...
                <span className={`text-xl font-bold ${changeRate != null && changeRate > 0 ? 'text-up' : changeRate != null && changeRate < 0 ? 'text-down' : 'text-subtle'}`}>
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              <span className="text-[26px] font-bold tracking-[-0.025em]">{portfolioWeight == null ? '-' : `${portfolioWeight.toFixed(1)}%`}</span>
              <span className="text-base text-muted">{portfolioAmount == null ? '-' : won(portfolioAmount)}</span>
            </div>
          </section>
          ...
          <section className="flex flex-col gap-7 rounded-card bg-surface p-12">
            <div className="flex items-center justify-between gap-6">
              ...
              <div className="flex gap-2">
                {TIMEFRAMES.map((item, index) => (
                  <button key={item.period} onClick={() => setTfIndex(index)} className={`rounded-full px-5 py-3 text-base font-semibold ${index === tfIndex ? 'bg-lime text-navy' : 'bg-[#F4F6F1] text-muted'}`}>
                    {item.label}
                  </button>
                ))}
              </div>
            ...
            <div className="flex items-start justify-between gap-8">
              <div className="flex flex-col gap-2">
                <span className="text-base text-muted">{timeframe.label} 변화</span>
                <span className={`text-[32px] font-bold tracking-[-0.03em] ${periodChange != null && periodChange > 0 ? 'text-up' : periodChange != null && periodChange < 0 ? 'text-down' : 'text-subtle'}`}>{signed(periodChange)}%</span>
              </div>
              {chartMode === 'detail' && (
                <div className="flex gap-8 text-[15px] text-muted">
                  <span>고가 <b className="text-ink">{high == null ? '-' : won(high)}</b></span>
                  <span>저가 <b className="text-ink">{low == null ? '-' : won(low)}</b></span>
                </div>
              )}
            </div>
            <div className="h-[300px] w-full">
              {isChartUnavailable(chartError, priceData) ? (
                <div className="flex h-full items-center justify-center text-[17px] text-muted">차트 데이터를 불러올 수 없습니다.</div>
              ) : (
                <ResponsiveContainer>
                  <LineChart data={priceData} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
                    ...
                    {chartMode === 'detail' && <YAxis domain={['dataMin', 'dataMax']} tick={{ fill: '#8A948C', fontSize: 13 }} axisLine={false} tickLine={false} width={70} />}
                    {chartMode === 'detail' && <Tooltip formatter={(value: number) => won(value)} labelFormatter={() => ''} />}
                    <Line type="monotone" dataKey="price" stroke={chartMode === 'detail' ? '#18243A' : '#3E5372'} strokeWidth={chartMode === 'detail' ? 3 : 5} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              )}
            </div>
            {chartMode === 'simple' && <p className="text-[17px] leading-7 text-muted">실제 {chart?.source ?? '시장'} 시세를 표시해요. 자세한 고가·저가와 거래량은 “자세하게 보기”에서 볼 수 있어요.</p>}
          </section>
          ...
          <section className="flex flex-col gap-6 rounded-card bg-surface p-12">
            ...
            <div className="grid grid-cols-5 gap-4">
              {metrics.map((item) => (
                <div key={item.label} className="flex flex-col gap-2.5 rounded-[18px] bg-canvas px-6 py-7">
                  <div className="flex items-center gap-2">
                    <span className="text-[15px] text-muted">{item.label}</span>
                    {item.key && <button aria-label={`${item.label} 설명`} onClick={() => setActiveTooltip((previous) => previous === item.key ? null : item.key)} className={`h-5 w-5 rounded-full text-xs font-bold ${activeTooltip === item.key ? 'bg-lime text-navy' : 'bg-[#EDEFEA] text-muted'}`}>?</button>}
                  </div>
                  <span className="text-2xl font-bold tracking-[-0.025em]">{item.value}</span>
                </div>
              ))}
            </div>
            {term && (
              <div className="flex flex-col gap-4 rounded-[20px] bg-[#F8FCEE] px-10 py-9">
                <div className="flex items-start justify-between gap-6">
                  <div className="flex flex-wrap items-baseline gap-2.5"><span className="text-[22px] font-bold tracking-[-0.02em]">{term.title}</span><span className="text-[17px] text-muted">{term.ko}</span></div>
                  <button aria-label="닫기" onClick={() => setActiveTooltip(null)} className="rounded-[9px] bg-surface p-2 text-[#3F4A43]"><X size={16} /></button>
                </div>
                <p className="max-w-[800px] text-lg leading-[30px] text-[#3F4A43]">{term.plain}</p>
                <div className="flex items-center gap-3 rounded-[14px] bg-surface px-6 py-5"><span className="shrink-0 text-[15px] font-semibold text-muted">계산식</span><span className="text-[17px] font-semibold text-ink">{term.formula}</span></div>
              </div>
            )}
          </section>
...
function Toggle({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return <button onClick={onClick} className={`rounded-full px-7 py-3.5 text-[17px] font-semibold ${active ? 'bg-surface text-ink shadow-[0_2px_8px_rgba(24,36,58,0.08)]' : 'text-muted'}`}>{children}</button>;
}
