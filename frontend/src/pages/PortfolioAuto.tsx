...
export default function PortfolioAuto({
  userName,
  onNavigate,
  onOpenDetail,
}: Props) {
  ...
  const principalTotal = portfolio
    ? Number(portfolio.total_purchase_amount)
    : accountMissing
      ? MOCK_PRINCIPAL_TOTAL
      : 0;
  const holdTotal = portfolio
    ? Number(portfolio.total_assets)
    : accountMissing
      ? MOCK_HOLD_TOTAL
      : 0;
  ...
  const [historyPeriod, setHistoryPeriod] =
    useState<PortfolioHistoryPeriod>("1Y"); // 기본값 "1년"
  ...
  // 실 계좌 응답이 있으면 실제 제안을 그대로 쓰고, 계좌 없음이 확정된 경우에만 목업을 쓴다.
  const displayAlerts = useMemo(
    () =>
      portfolio
        ? getDisplayAlerts(portfolio)
        : accountMissing
          ? getDisplayAlerts(null)
          : [],
    [accountMissing, portfolio],
  );
  ...
  const recentTransactions = useMemo(
    () =>
      getDisplayTransactions(
        executions,
        account !== null || !accountMissing,
      ).slice(0, 3),
    [executions, account, accountMissing],
  );
  ...
  const trendData = useMemo(() => {
    if (account) {
      return (history?.items ?? []).map((item) => ({
        ...
        kospi:
          item.benchmark_return_rate == null
            ? null
    ...
    return [];
  }, [account, accountMissing, history, historyPeriod]);
  ...
  const contributionData = useMemo(() => {
    if (portfolio) {
      return [...portfolio.contributions]
        .map((c) => ({
          name: c.stock_name ?? c.stock_code,
  ...
  const renderDonutChart = (sizeClass: string, onExpand?: () => void) => (
    <div className={`relative donut-no-focus-outline ${sizeClass}`}>
              ...
              <Cell
                ...
                fill={
                  i === selectedHoldingIdx
                    ? "#C6F04D"
                    : DONUT_SHADES[i % DONUT_SHADES.length]
                }
                cursor="pointer"
              />
            ))}
      ...
      <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center gap-1 px-6 text-center">
        {(() => {
          const centerIdx =
            hoverHoldingIdx ?? (hasSelectedHolding ? selectedHoldingIdx : null);
          if (!hasHoldings) {
          ...
          return (
            <>
              ...
              <span
                className={`text-2xl font-bold tracking-[-0.03em] ${chg >= 0 ? "text-up" : "text-down"}`}
              >
                {centerHolding.pct.toFixed(1)}%
              </span>
              {/* 수익률에는 +/- 부호를 그대로 노출한다(양수는 +, 음수는 - 가 toFixed 에 이미 포함됨) */}
              <span
                className={`text-xs font-bold ${chg >= 0 ? "text-up" : "text-down"}`}
              >
                {chg >= 0 ? "+" : ""}
                {chg.toFixed(1)}%
              </span>
      ...
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm text-subtle">
            비교 기준 <b className="text-[#3F4A43]">{benchmarkName}</b>
          </span>
          {onExpand && (
            <button
              aria-label="차트 크게 보기"
      ...
      <div className="min-h-0 flex-1">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Legend
              iconType="plainline"
              wrapperStyle={{ fontSize: 13, color: "#5C665F" }}
            />
    ...
    <div className={`flex flex-col gap-3 ${heightClass}`}>
      <div className="flex shrink-0 items-center justify-between gap-3">
        <p className="text-sm text-subtle">
          최근 1개월 동안 각 종목이 전체 수익에 얼마나 영향을 줬는지 보여줘요.
        </p>
        {onExpand && (
          <button
            aria-label="차트 크게 보기"
  ...
  return (
    ...
    <>
                ...
                <div className="relative flex min-h-[420px] flex-col gap-3 lg:min-h-0 lg:pb-14">
                  {tab === "trend" && (
                    <div className="flex h-full flex-col gap-3">
                      ...
                      {renderTrendChart("h-full", () =>
                        setIsChartZoomOpen(true),
                      )}
                      {trendData.length === 0 ? (
                        <Insight compact>
                          아직 표시할 자산 변화 데이터가 없어요.
                        </Insight>
                      ) : (
                        (() => {
                  ...
                  {tab === "contribution" && (
                    <div className="flex h-full flex-col gap-3">
                      ...
                      {renderContributionChart("h-full", () =>
                        setIsChartZoomOpen(true),
                      )}
                      <Insight compact>
                        {topContributor
                          ? `${topContributor.name}가 수익에 가장 많이 기여했어요.`
                          : "아직 표시할 기여도 데이터가 없어요."}
                      </Insight>
                    </div>
                  )}
                  ...
                  {tab === "weight" && (
                    <div className="flex h-full flex-col items-center justify-center gap-4">
                      ...
                      {renderDonutChart(
                        "h-full max-h-[420px] w-full max-w-[420px]",
                        () => setIsChartZoomOpen(true),
                      )}
                ...
                <div className="flex min-h-0 w-full flex-col gap-4 self-center rounded-[20px] bg-canvas px-8 py-6">
                  ...
                  <div className="flex shrink-0 flex-col gap-1.5">
                    ...
                    <div className="flex flex-wrap items-baseline gap-3">
                      <span className="text-[32px] font-bold tracking-[-0.02em] text-ink">
                        {won(holdTotal)}
                      </span>
                      {/* 증감액 + 증감률 — 이 앱의 다른 화면과 동일하게 상승은 text-up, 하락은 text-down 으로 표시한다 */}
                      <span
                        className={`text-lg font-bold ${gainPct >= 0 ? "text-up" : "text-down"}`}
                      >
                        {gainAmount >= 0 ? "+" : ""}
                        {gainAmount.toLocaleString("ko-KR")}원 (
                        {gainPct >= 0 ? "+" : ""}
                        {gainPct.toFixed(1)}%)
                      </span>
                    </div>
                  ...
                  <div className="flex shrink-0 flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-2 rounded-[16px] bg-surface p-4">
                          ...
                          {displayAlerts.slice(0, 3).map((a) => (
                            ...
                            >
                              <div className="flex w-full items-center justify-between gap-1">
                                <span
                                  className={`w-fit whitespace-nowrap rounded-full px-2 py-0.5 text-[11px] font-bold ${ALERT_BADGE[a.kind]}`}
                                >
                                  {a.badge}
                                </span>
                                ...
                                {!usingRealAlerts && (
                                  <Check
                                    size={12}
                                    className="shrink-0 text-[#3F5222]"
                                    aria-label="완료"
                                  />
                                )}
                              </div>
                      ...
                      <button
                        onClick={onOpenDetail}
                        className="self-end text-xs font-bold text-navy"
                      >
                        더 알아보기 →
                      </button>
                    </div>
                      ...
                      {recentTransactions.length === 0 ? (
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          {recentTransactions.map((t) => (
                            <div
                              key={t.id}
                              className="flex min-w-0 flex-col gap-1.5 rounded-[10px] bg-canvas px-2.5 py-2.5"
                            >
                              <span
                                className={`w-fit whitespace-nowrap rounded-full px-2 py-0.5 text-[11px] font-bold ${TX_BADGE[t.type]}`}
                              >
                                {t.type}
                              </span>
                              <span className="w-full truncate text-xs font-semibold text-ink">
                              ...
                              <span
                                className={`w-full truncate text-xs font-bold ${t.amount >= 0 ? "text-up" : "text-down"}`}
                              >
                                {t.amount >= 0 ? "+" : ""}
      ...
      {alertModal && (
        ...
        >
          <div
            className="flex w-[480px] flex-col gap-5 rounded-card bg-surface p-9"
          ...
          >
            <div className="flex items-start justify-between gap-6">
              <div className="flex flex-col gap-2">
                ...
                <h2 className="text-xl font-bold tracking-[-0.02em]">
                  {alertModal.stockName} ·{" "}
                  {usingRealAlerts ? "왜 지금인가요?" : "왜 실행했나요?"}
                </h2>
              </div>
              <button
                aria-label="닫기"
            ...
            <div className="flex items-center gap-3 rounded-[14px] bg-[#F8FCEE] px-6 py-5">
              <span className="shrink-0 text-sm font-semibold text-[#3F5222]">
                {usingRealAlerts ? "AI 제안" : "AI 조치"}
              </span>
              <span className="text-sm font-semibold text-ink">
                {alertModal.action}
              </span>
...
function Insight({
  ...
  return (
    <div
      className={`flex shrink-0 items-start gap-3 rounded-[16px] bg-[#F8FCEE] ${compact ? "px-6 py-3" : "px-8 py-6"}`}
    >
      <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-lime text-sm text-navy">
        ✦
      </div>
