...
export default function TransactionDetail({
  ...
  const transactions = useMemo(
    () =>
      getDisplayTransactions(executions, account !== null || !accountMissing),
          ...
          {!t ? (
            ...
            <>
              <section className="flex flex-col gap-4">
                <span
                  className={`w-fit rounded-full px-3 py-1.5 text-sm font-bold ${TX_BADGE[t.type]}`}
                >
                  {t.type}
                </span>
                <h1 className="text-[34px] font-bold leading-[46px] tracking-[-0.03em]">
                  {t.stockName}
                </h1>
                <span
                  className={`text-[28px] font-bold tracking-[-0.03em] ${t.amount >= 0 ? "text-up" : "text-down"}`}
                >
                  {t.amount >= 0 ? "+" : ""}
              ...
              <section className="flex flex-col gap-0 rounded-card bg-surface p-2">
                ...
                <Row
                  label="수량"
                  value={t.quantity > 0 ? `${t.quantity}주` : "-"}
                />
                <Row
                  label="체결 단가"
                  value={
                    t.price > 0 ? `${t.price.toLocaleString("ko-KR")}원` : "-"
                  }
                />
                <Row
                  label="수수료"
                  value={`${t.fee.toLocaleString("ko-KR")}원`}
                />
...
function Row({
  ...
  return (
    <div
      className={`flex items-center justify-between px-6 py-5 ${last ? "" : "border-b border-line"}`}
    >
